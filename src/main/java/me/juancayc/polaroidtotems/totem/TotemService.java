...
public final class TotemService {
    ...
    public void applyResurrection(Player player, FoundTotem found, boolean consumeManually) {
        ...
        scheduleEffects(player, definition);
    }

    /**
     * Applies a type's custom effects one tick later.
     *
     ...
     */
    private void scheduleEffects(Player player, TotemDefinition definition) {
        if (definition.effects().isEmpty()) return;
